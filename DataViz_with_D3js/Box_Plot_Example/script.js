...
d3.csv("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/iris.csv").then((data, error) => {
  ...
  d3.select(mySelection).append("span").append("p").attr("class", "label").text("Select attribute to analyze:").style("font-weight", "bold").style("font-size", "21px");

  const selectItems = ["Sepal_Length", "Sepal_Width", "Petal_Length", "Petal_Width"];
